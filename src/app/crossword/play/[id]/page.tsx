...
export default function CrosswordPlayPage({ params }: { params: { id: string; code: string } }) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const storedSession = localStorage.getItem(`crossword_player_${params.id}`);
        if (storedSession) {
          ...
          if (sessionData) {
            ...
            if (playerSession) {
              ...
              localStorage.removeItem(`crossword_player_${params.id}`);
            }
          } else {
            // Clear invalid session from local storage
            localStorage.removeItem(`crossword_player_${params.id}`);
