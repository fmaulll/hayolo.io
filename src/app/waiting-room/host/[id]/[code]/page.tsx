...
export default function WaitingRoomHost({ params }: { params: { id: string; code: string } }) {
  ...
  const setupHostSession = async () => {
    try {
      ...
      const storedSession = localStorage.getItem(`crossword_host_${params.code}`);
      if (storedSession) {
      ...
      if (playerSession && sessionData) {
        localStorage.setItem(`crossword_host_${params.code}`, JSON.stringify({
  ...
  return (
    <div className="min-h-screen bg-white bg-[url('/background-seamless.png')] bg-repeat py-16">
